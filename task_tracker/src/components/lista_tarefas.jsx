...
export default function ListaTarefas ({tarefas, onDeletar, onEditar, onAlternarStatus}) {
  const style = {
      py: 0,
  width: '100%',
  maxWidth: 360,
  ...
  return(
      <List sx={style}>
        {tarefas.map((tarefa, indice) =>(
          <Box key={tarefa.id}>
            <TarefaItem tarefa={tarefa} onDeletar ={onDeletar} onEditar={onEditar}/>
            {indice < tarefas.length - 1 && <Divider />}
          </Box>
        ))};
      </List>
  )
